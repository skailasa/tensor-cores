...
double performance_metrics(std::ofstream &fs, int M, int N, int K,
                           float time_kernel, float time_cublas);

// Template function definitions (define here, since templates must be visible
// to all TUs)
template <typename T> constexpr int count_memory(int M, int N, int K) {
...
template <typename T> void zero_init_matrix(T *mat, int size) {
  for (int i = 0; i < size; i++) {
    mat[i] = 0.0;
  }
}
...
template <typename T>
double compute_relative_error_fro(const T *A, const T *B, int M, int N,
                                  Layout layout) {
