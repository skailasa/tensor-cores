...
int main() {
    ...
    std::vector<double> Dref_h(D_size);
        ...
        double *A_d, *B_d, *D_d;
        cudaMalloc(&A_d, A_size * sizeof(double));
        cudaMalloc(&B_d, D_size * sizeof(double));
        cudaMalloc(&D_d, D_size * sizeof(double));

        cudaMemcpy(A_d, A_h.data(), A_size * sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(B_d, B_h.data(), B_size * sizeof(double), cudaMemcpyHostToDevice);

        dgemm<<<grid, block>>>(A_d, B_d, D_d);

        std::vector<double> D_h(D_size);
        cudaMemcpy(D_d, D_h.data(), D_size * sizeof(double), cudaMemcpyDeviceToHost);
