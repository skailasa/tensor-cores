...
int main() {
  ...
  auto time_cublas = runKernel32(0, layout, cache_configuration, M, N, K, alpha,
                                 A_d, B_d, beta, C_d);
  auto time_kernel = runKernel32(9, layout, cache_configuration, M, N, K,
  ...
  // Copy back result
  CUDA_CHECK(
      cudaMemcpy(A, A_d, (M * K) * sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK(
      cudaMemcpy(B, B_d, (K * N) * sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK(
