...
// equivalent to typedef void (*my_kernel)(arg1, arg2,...) - a raw pointer to a
// function can point to any function with this/that signature
using KernelPtr = void (*)(int, int, int, float, const float *, const float *,
                           float, float *);

/// @brief  Call cubLAS with single precision inputs
void runCublasF32(cublasHandle_t handle, Layout layout, int M, int N, int K,
                  float alpha, float *A, float *B, float beta, float *C) {

  if (layout == Layout::RowMajor) {
    cublasSgemm_v2(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, B, N, A,
                   K, &beta, C, N);
  } else if (layout == Layout::ColumnMajor) {
    cublasSgemm_v2(handle, CUBLAS_OP_N, CUBLAS_OP_N, M, N, K, &alpha, A, M, B,
                   K, &beta, C, M);
  }
}

/// @brief  Call cubLAS with single precision inputs casted down to BF16 for the
/// actual mul
void runCublasB16(cublasHandle_t handle, Layout layout, int M, int N, int K,
                  float alpha, float *A, float *B, float beta, float *C) {
  if (layout == Layout::RowMajor) {
...
/// @brief  Call cubLAS with single precision inputs casted to TF32 for the
/// actual mul
void runCublasT32(cublasHandle_t handle, Layout layout, int M, int N, int K,
                  float alpha, float *A, float *B, float beta, float *C) {
  if (layout == Layout::RowMajor) {
    cublasGemmEx(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, B,
                 CUDA_R_32F, N, A, CUDA_R_32F, K, &beta, C, CUDA_R_32F, N,
...
void runSGemmNaive(Layout layout, cudaFuncCache cache_configuration, int M,
...
void runSgemmSharedMemCacheBlocking(Layout layout,
                                    cudaFuncCache cache_configuration, int M,
...
void runSgemm1dBlockTiling(Layout layout, cudaFuncCache cache_configuration,
  ...
  if (layout == Layout::RowMajor) {
    ...
    dim3 blockDim(
        (BM * BN) /
        TM); // each thread in block responsible for TM values of output
...
void runSgemm2dBlockTiling(Layout layout, cudaFuncCache cache_configuration,
                           int M, int N, int K, float alpha, float *A, float *B,
                           float beta, float *C) {
  ...
  if (layout == Layout::RowMajor) {
    ...
    // cache blocking, but with tunable parameters KernelPtr kernel =
    // sgemm_smem_2d_blocktiling_column_major<BM, BN, BK, TM, TN>;
    // cudaFuncSetCacheConfig(kernel, cache_configuration);
...
void runSgemmVectoriseSmem(Layout layout, cudaFuncCache cache_configuration,
  ...
  if (layout == Layout::RowMajor) {
    ...
    dim3 blockDim((BM * BN) / ((TM * TN)));
    KernelPtr kernel =
        sgemm_vectorise_smem_and_gmem_accesses_row_major<BM, BN, BK, TM, TN>;
    cudaFuncSetCacheConfig(kernel, cache_configuration);
    kernel<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
    ...
    // KernelPtr kernel =
    //     sgemm_vectorise_smem_and_gmem_accesses_column_major<BM, BN, BK, TM,
    //     TN>;
    // cudaFuncSetCacheConfig(kernel, cache_configuration);
    // kernel<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
  }
...
void runSgemmWarptiling(Layout layout, cudaFuncCache cache_configuration, int M,
                        int N, int K, float alpha, float *A, float *B,
                        float beta, float *C) {
  ...
  constexpr int threadsPerSubtileRow = WSUBM / TM;
...
float runKernel32(int kernel_number, Layout layout,
  ...
  case 4:
    ...
    time = run_kernel_with_optional_timing(
        [=]() {
          runSGemmNaive(layout, cache_configuration, M, N, K, alpha, A, B, beta,
                        C);
        },
        true);
