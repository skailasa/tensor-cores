...
__global__ void sgemm_smem_1d_blocktiling_row_major(int M, int N, int K,
  ...
  // Need to still load As and Bs tiles into shared memory by looping along
  // shared dimension
  for (int bk = 0; bk < K; bk += BK) {

    // Load As

    // Only threads with tid UP TO BM*BK will participate, so at worst one
    // thread will be responsible for loading one element
    ...
    // i.e. loop over rows of As, same index as result index. Coalesced loads
    // from B Non coalesced loads from As
    for (int k = 0; k < BK; ++k) {
  ...
  for (int resIdx = 0; resIdx < TM; ++resIdx) {
    ...
    C[globalRow * N + globalCol] =
        alpha * threadResults[resIdx] + beta * C[globalRow * N + threadCol];
  }
}

template <const int BM, const int BN, const int BK, const int TM>
__global__ void
sgemm_smem_1d_blocktiling_column_major(int M, int N, int K, float alpha,
  ...
  // This threads result, still responsible for column of TM of output matrix C
  // per thread
  float threadResults[TM] = {0.0f};

  // Now running this over columns of each output matrix, rather than rows like
  // above
  int threadId = threadIdx.x;
  int threadsPerBlock = blockDim.x;
  ...
  for (int resIdx = 0; resIdx < TM; ++resIdx) {
    ...
    C[globalRow + globalCol * M] =
        alpha * threadResults[resIdx] + beta * C[globalRow + globalCol * M];
  }
}

template __global__ void sgemm_smem_1d_blocktiling_row_major<64, 64, 16, 4>(
    int, int, int, float, const float *, const float *, float, float *);
template __global__ void sgemm_smem_1d_blocktiling_column_major<64, 64, 16, 4>(
    int, int, int, float, const float *, const float *, float, float *);
