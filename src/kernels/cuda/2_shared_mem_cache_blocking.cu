...
__global__ void sgemm_smem_cache_blocking_row_major(int M, int N, int K,
  ...
  // Loop along shared dimension, each thread block loads in a tile of BLOCKSIZE
  // * BLOCKSIZE from each of A and B, in a coalesced manner, into shared memory
  // As and Bs resp.
  float tmp = 0.0;
...
template <const int BLOCKSIZE>
__global__ void
sgemm_smem_cache_blocking_column_major(int M, int N, int K, float alpha,
                                       const float *A, const float *B,
                                       float beta, float *C) {

  // The inner (block wise) row and column we're accessing in this thread
  const uint localCol = threadIdx.y;
  const uint localRow = threadIdx.x; // we now want the fastest running index to
                                     // be for rows, for coalescing purposes
  ...
  if (globalRow < M && globalCol < N)
    C[globalCol * M + globalRow] =
        alpha * tmp + beta * C[globalCol * M + globalRow];
}

template __global__ void
sgemm_smem_cache_blocking_row_major<32>(int, int, int, float, const float *,
                                        const float *, float, float *);

template __global__ void
sgemm_smem_cache_blocking_column_major<32>(int, int, int, float, const float *,
                                           const float *, float, float *);